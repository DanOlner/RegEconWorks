...
There are two interactive web pages for each of the GVA sources discussed below. I'll refer back to these with links.
...
The closer decisions get to regions, however, the less that holds up. Why? Spuriously accurate point data masks the need for deeper regional knowledge structures, built bottom-up and connected into decision-making.
...
This not only builds a much richer picture (that can include uncertain data but doesn't rely on it), that sense-checking process is precisely catches mistakes
...
Generally, decision-making has a type I 'error management' bias (false alarms matter more):

> "We sometimes think sticks are snakes (which is harmless), but rarely that snakes are sticks (which can be deadly)." @johnson2013

streetlight effect [@kirman1992 p.p.134]
...
Let's return to the 'avoid chasing vampires off cliffs' idea. This 'be afraid of type II errors' approach is embodied in the precautionary principle: "extra precaution is justified when false negatives are worse than false positives" @persson2016.
